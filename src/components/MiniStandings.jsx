...
import { TEAM_ID, DIVISION, SPONSORS } from '../config.js'
...
// Compact NL Central standings for sidebar/in-article embeds. The whole card is one link into
...
export default function MiniStandings() {
  ...
  const band = (text) => (
    <div style={{ background: theme.navy, color: '#fff', fontSize: 9.5, letterSpacing: '0.18em', fontWeight: 700, textTransform: 'uppercase', padding: '5px 8px', textAlign: 'center' }}>{text}</div>
  )
  const footer = (
  ...
  if (!standings) {
    return (
      <a {...linkProps} className="mini-card" style={card}>
        {band('NL Central standings')}
        <div style={{ padding: '16px 14px', fontFamily: theme.serif, fontSize: 16, color: theme.ink, textAlign: 'center' }}>The Brewers, by the numbers</div>
        {footer}
      </a>
    )
  }
              ...
              return (
                <tr key={t.team.id} style={me ? { background: theme.wash } : undefined}>
                  <td style={{ ...td, textAlign: 'left' }}>
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 7, fontFamily: theme.serif, fontSize: 14, fontWeight: me ? 700 : 400, color: me ? theme.navy : theme.ink }}>
                      <TeamLogo id={t.team.id} size={18} />
