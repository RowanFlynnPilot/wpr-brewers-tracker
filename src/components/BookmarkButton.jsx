...
// such shortcut, so a one-tap "copy link" (the canonical WPR Brewers page) is the reliable path
// there. Pure client UI; every branch fails soft.

const isMac = typeof navigator !== 'undefined' && /Mac|iPhone|iPad|iPod/.test(navigator.platform || navigator.userAgent || '')
const isTouch = typeof window !== 'undefined' && !!window.matchMedia && window.matchMedia('(pointer: coarse)').matches

// The URL a bookmark/copy should point at: the dedicated WPR Brewers page when embedded (so
...
const Key = ({ children }) => (
  <kbd style={{ fontFamily: theme.sans, fontSize: 11, fontWeight: 700, color: theme.navy, background: theme.wash, border: `1px solid ${theme.rule}`, borderRadius: 4, padding: '2px 6px', margin: '0 2px', boxShadow: '0 1px 0 rgba(0,0,0,0.06)' }}>{children}</kbd>
)
  ...
  return (
    <div ref={wrap} style={{ position: 'relative', flexShrink: 0 }}>
      <button onClick={toggle} aria-expanded={open} aria-label="Bookmark this page" style={{
        ...
        fontFamily: theme.sans, fontSize: 11, fontWeight: 700, letterSpacing: '0.04em', color: theme.navy,
      }}>
        <span style={{ color: theme.gold, fontSize: 12, lineHeight: 1 }}>★</span> Bookmark
      </button>
      ...
      {open && (
        <div role="dialog" aria-label="Bookmark this page" style={{
          ...
          boxShadow: '0 10px 30px rgba(18,40,75,0.18)', padding: '14px 16px',
        }}>
          <div style={{ fontFamily: theme.serif, fontSize: 16, color: theme.ink, marginBottom: 5 }}>Keep the Brewers handy</div>
          ...
          <button onClick={copy} style={{
            ...
            background: copied ? theme.navy : 'transparent', color: copied ? '#fff' : theme.navy,
            border: `1px solid ${copied ? theme.navy : theme.rule}`, borderRadius: 7, padding: '8px 10px',
