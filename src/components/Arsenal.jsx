...
function Donut({ mix, total }) {
  ...
  let offset = 0
  return (
    <svg viewBox="0 0 110 110" width="110" height="110" role="img" aria-label="Pitch usage by type">
      <g transform="rotate(-90 55 55)">
        {mix.map((m) => {
          const len = (m.n / total) * C
          const seg = (
            <circle key={m.code} cx="55" cy="55" r={R} fill="none" stroke={pitchColor(m.code)} strokeWidth="16"
      ...
      <text x="55" y="51" textAnchor="middle" fontFamily={theme.serif} fontSize="22" fontWeight="700" fill={theme.navy}>{total}</text>
      <text x="55" y="66" textAnchor="middle" fontFamily={theme.sans} fontSize="9" fill={theme.muted}>pitches</text>
    </svg>
  )
}
...
function VeloChart({ pitches }) {
  ...
  const W = 300, H = 120, padL = 28, padB = 16, padT = 8, padR = 6
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ maxWidth: 320, display: 'block' }} role="img" aria-label="Velocity by pitch number">
      {[vMin, Math.round((vMin + vMax) / 2), vMax].map((v) => (
        <g key={v}>
          <line x1={padL} y1={y(v)} x2={W - padR} y2={y(v)} stroke={theme.rule} strokeWidth="0.75" />
          <text x={padL - 4} y={y(v) + 3} textAnchor="end" fontFamily={theme.sans} fontSize="8" fill={theme.muted}>{v}</text>
        </g>
      ))}
      {withV.map((p, i) => <circle key={i} cx={x(pitches.indexOf(p))} cy={y(p.velo)} r="2.6" fill={pitchColor(p.code)} />)}
      <text x={padL} y={H - 3} fontFamily={theme.sans} fontSize="8" fill={theme.muted}>first pitch</text>
      <text x={W - padR} y={H - 3} textAnchor="end" fontFamily={theme.sans} fontSize="8" fill={theme.muted}>last</text>
    </svg>
  )
}
