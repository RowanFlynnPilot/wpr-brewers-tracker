...
import { fetchBrewersCoverage } from '../wpr.js'
        ...
        {posts.map((p) => (
          ...
          >
            ...
            <div style={{ padding: '11px 13px 13px', display: 'flex', flexDirection: 'column', gap: 5 }}>
              <div style={{ fontFamily: theme.sans, fontSize: 10, letterSpacing: '0.1em', textTransform: 'uppercase', color: theme.gold, fontWeight: 700 }}>{date(p.date)}</div>
      ...
      {WPR_NEWS?.archive && (
        <div style={{ marginTop: 16 }}>
          <a href={WPR_NEWS.archive} target="_top" className="link-hover" onClick={() => track('Coverage Click', { title: 'archive' })}
            style={{ fontFamily: theme.sans, fontSize: 12, fontWeight: 700, color: theme.navy, textDecoration: 'none' }}>
            More Brewers coverage at Wausau Pilot &amp; Review {'→'}
          </a>
        </div>
      )}
    </div>
