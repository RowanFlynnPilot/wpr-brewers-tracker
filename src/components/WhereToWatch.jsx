  ...
  return (
    <div style={{ border: `1px solid ${theme.rule}`, borderLeft: `3px solid ${theme.gold}`, borderRadius: 10, background: theme.wash, overflow: 'hidden', display: 'flex', flexDirection: narrow ? 'column' : 'row' }}>
      ...
      <div style={{ flex: 1, padding: '18px 20px 20px' }}>
        <div style={{ fontFamily: theme.sans, fontSize: 10, letterSpacing: '0.16em', textTransform: 'uppercase', color: theme.gold, fontWeight: 700 }}>Presented by</div>
        ...
        {venue.features?.length > 0 && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7, marginTop: 12 }}>
            {venue.features.map((f) => (
              <span key={f} style={{ border: `1px solid ${theme.rule}`, background: '#fff', borderRadius: 20, padding: '3px 11px', fontFamily: theme.sans, fontSize: 11.5, color: theme.navy, fontWeight: 700 }}>{f}</span>
            ))}
          </div>
        )}
        ...
        {venue.url && (
          <a {...linkProps} className="link-hover" style={{ display: 'inline-block', marginTop: 14, fontFamily: theme.sans, fontSize: 12, fontWeight: 700, color: theme.navy, textDecoration: 'none' }}>
            Visit {venue.name} {'→'}
          </a>
        )}
      </div>
