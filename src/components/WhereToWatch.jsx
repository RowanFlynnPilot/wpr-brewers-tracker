...
import { WATCH_VENUES, SPONSOR_INQUIRY, TEAM_ID } from '../config.js'
...
// section to the next actual first pitch (one schedule read, fail-soft). Owns its Section;
// with no venues sold there is no section — except in ?demo, where sales previews fill it
// with placeholders (see config.js).
...
function VenueCard({ venue }) {
  ...
  return (
    <div style={{ border: `1px solid ${theme.rule}`, borderTop: `3px solid ${theme.gold}`, borderRadius: 10, background: '#fff', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ aspectRatio: '16 / 9', background: theme.navy, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        {hero && !heroFailed ? (
      ...
      {thumbs.length > 0 && (
        <div style={{ display: 'flex', gap: 3, background: theme.navy }}>
          {thumbs.map((t, i) => (
      ...
      <div style={{ padding: '15px 18px 17px', display: 'flex', flexDirection: 'column', flex: 1 }}>
        <div style={{ fontFamily: theme.sans, fontSize: 9.5, letterSpacing: '0.16em', textTransform: 'uppercase', color: theme.gold, fontWeight: 700 }}>Game-day partner</div>
        ...
        {venue.features?.length > 0 && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7, marginTop: 12 }}>
            {venue.features.map((f) => (
              <span key={f} style={{ border: `1px solid ${theme.rule}`, background: theme.wash, borderRadius: 20, padding: '3px 11px', fontFamily: theme.sans, fontSize: 11.5, color: theme.navy, fontWeight: 700 }}>{f}</span>
            ))}
          </div>
        )}
        ...
        {venue.url && (
          <a {...linkProps} className="link-hover" style={{ display: 'inline-block', marginTop: 'auto', paddingTop: 14, fontFamily: theme.sans, fontSize: 12, fontWeight: 700, color: theme.navy, textDecoration: 'none' }}>
            Menu &amp; info {'→'}
          </a>
        )}
      </div>
...
export default function WhereToWatch() {
  ...
  useEffect(() => {
    ...
    let alive = true
    fetchFeaturedGame()
      .then((g) => { if (alive && g?.status?.abstractGameState === 'Preview') setNext(g) })
      .catch(() => {})
    return () => { alive = false }
  }, [])
  ...
  const intro = next && (() => {
    ...
    const opp = (home ? next.teams.away : next.teams.home).team
    const when = new Date(next.gameDate).toLocaleString('en-US', { weekday: 'long', hour: 'numeric', minute: '2-digit' })
    return `Brewers ${home ? 'vs' : 'at'} the ${opp.teamName || opp.name}, ${when} — here's where Wausau will be watching.`
  })()
