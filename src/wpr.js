// Wausau Pilot & Review's own Brewers coverage, via the WordPress REST API. Third external API,
// same rules as the MLB + weather clients: browser fetch, keyless, CORS-open, fail fast (the
...
// Recent Brewers articles, newest first. `_fields` trims the payload (no full article body);
// `_embed` carries the featured-media thumbnail.
export async function fetchBrewersCoverage(limit = 4) {
  if (!WPR_NEWS) return []
